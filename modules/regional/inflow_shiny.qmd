---
title: Relocations from the region
format: html
server: shiny
---
...
pop_est = read.csv('../../newdata/GHA_pop_estimates_2022_2024.csv')
relocations = read.csv('../../newdata/GHA_reloc_estimates_2020_2024.csv') %>%
...
library(ggraph)
  ...
  selected_region_centroid <- reactive({
    selected_region_centroid <- regional_map() %>%
  ...
  output$flow_map <- renderPlot({
    ...
    #     aes(x = x, y = y, group = id, color = gradient, size = flow),
